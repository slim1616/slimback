    ...
    <div class="page-inner">
        ...
        <div class="card">
            <div class="card-header">
                <div class="card-title">Modifier departement</div>
            </div>
            <div class="card-body">      
                <form @submit.prevent="updateDepartement" v-if="loaded">
                  ...
                  <div class="row">
                    ...
                    <div class="card-action">
                        ...
                        <button class="btn btn-danger" @click.prevent="deleteDepartement">{{ (form.busy) ? 'Please wait...' : 'Effacer'}}</button>
                    </div>
                    
              </form>
              <span v-else>Loading departement...</span>
            </div>
        </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDepartement: function(Departement){
      
      var that = this;
      this.form.get('/api/departements/'+this.$route.params.id).then(function(response){
    ...
    updateDepartement: function(){
      var that = this;
        ...
        this.form.put('/api/departements/'+this.$route.params.id)
    ...
    deleteDepartement: function(){
      ...
      swal({
                ...
                if (Delete) {
                  this.$store.dispatch('setLoader', true)
                  this.form.delete('/api/departements/'+this.$route.params.id).then(function(response){
                    that.$store.dispatch('setLoader', false)
                      that.form.fill(response.data);
